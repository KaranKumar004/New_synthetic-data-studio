...
export default function Dashboard({ setCurrentTab, user }: DashboardProps) {
  ...
  return (
    <div className="flex-1 p-6 sm:p-8 lg:p-12 max-w-7xl xl:max-w-[90%] 2xl:max-w-[95%] mx-auto w-full space-y-8 pb-24 md:pb-12">
      {/* Welcome banner */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 glass-panel border border-card-border p-8 sm:p-10 rounded-3xl">
        <div>
        ...
        <button
          ...
          className="inline-flex items-center justify-center gap-2 px-6 py-3.5 bg-primary hover:bg-primary-hover text-white text-sm font-bold rounded-xl transition-all shadow-lg shadow-primary/10 hover:shadow-primary/20 cursor-pointer"
        >
      ...
      {stats && usagePercentage >= 80 && stats.plan === "free" && (
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 rounded-3xl bg-gradient-to-r from-indigo-500/15 via-violet-500/10 to-indigo-500/15 border border-indigo-500/30 animate-fade-in-up">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-5 py-3 bg-gradient-to-r from-indigo-500 to-violet-500 hover:from-indigo-600 hover:to-violet-600 text-white text-xs font-black rounded-xl transition-all shadow-md shadow-indigo-500/20 cursor-pointer shrink-0"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="glass-panel glass-panel-hover border border-card-border p-8 rounded-3xl flex flex-col items-center justify-center text-center">
          <h2 className="text-xs font-bold uppercase tracking-widest text-muted mb-6 self-start">
            Monthly Row Usage
          </h2>
        ...
        <div className="glass-panel glass-panel-hover border border-card-border p-8 rounded-3xl flex flex-col">
        ...
        <div className="grid grid-cols-2 gap-5">
          <div className="glass-panel glass-panel-hover border border-card-border p-6 rounded-3xl flex flex-col justify-between">
      ...
      <div className="glass-panel border border-card-border rounded-3xl p-8">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-border text-xs text-muted font-bold uppercase tracking-wider">
                  ...
                  <th className="pb-4 font-bold">Rows</th>
                  <th className="pb-4 font-bold">Format</th>
              ...
              <tbody className="divide-y divide-border text-sm font-medium">
                {stats.recent_activity.map((item: any) => (
                  <tr key={item.id} className="group hover:bg-muted-bg/30">
                    <td className="py-4.5 pr-3 text-foreground font-bold">{item.name}</td>
                    <td className="py-4.5 pr-3 text-muted">{item.row_count.toLocaleString()} rows</td>
                    ...
                    <td className="py-4.5 pr-3 text-muted">
                      {new Date(item.created_at).toLocaleDateString()}
                    </td>
                    <td className="py-4.5 text-right">
                      <button
                        onClick={() => handleDownload(item.id, item.name, item.file_format)}
                        className="inline-flex items-center gap-1.5 text-xs text-primary font-bold hover:bg-primary/10 px-3.5 py-2 rounded-lg border border-primary/20 transition-all cursor-pointer shadow-xs"
                      >
